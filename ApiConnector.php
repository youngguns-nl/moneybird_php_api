...
class ApiConnector
{
    ...
    protected function request($url, $method, $data = null)
    {
        ...
        try {
            ...
            if (self::$debug) {
                printf(
                    'Url: %s' . PHP_EOL . 'Method: %s' . PHP_EOL . 'Data: %s', $url, $method, $data
                );
            }
    ...
    public function save(Storable $model, AbstractModel $parent = null)
    {
        $response = $this->request(
            $this->buildUrl($model, $parent), $model->getId() > 0 ? 'PUT' : 'POST', $this->mapper->mapToStorage($model)
        );
    ...
    public function send(Sendable $model, AbstractEnvelope $envelope)
    {
        ...
        $response = $this->request(
            $this->buildUrl($model, null, '/send_' . strtolower($classname)), 'PUT', $this->mapper->mapToStorage($envelope)
        );
    ...
    public function registerPayment(Payable $invoice, AbstractPayment $payment)
    {
        ...
        $response = $this->request(
            $this->buildUrl($invoice, null, '/payments'), 'POST', $this->mapper->mapToStorage($payment)
        );
    ...
    public function remind(Invoice $invoice, InvoiceEnvelope $envelope)
    {
        ...
        $response = $this->request(
            $this->buildUrl($invoice, null, '/send_reminder'), 'PUT', $this->mapper->mapToStorage($envelope)
        );
    ...
    public function getByIds($classname, Array $ids)
    {
        ...
        $response = $this->request(
            $this->buildUrl(new $classname(), null, '/sync_fetch_ids'), 'POST', $this->mapper->mapToStorage($objects)
        );
        return $this->mapper->mapFromStorage($response);
    }
